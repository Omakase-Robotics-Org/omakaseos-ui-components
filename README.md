...
| **Direct-manipulation** | v0.16 | The path/polygon editing grammar and its visual vocabulary, shared by every map-editing surface in the suite (the robot console's 2D vendor scene, the dashboard's 3D waypoint editor). The headless kernel — geometry, hit-testing, the probe→affordance→intent grammar, the undo/redo session, and the `useDirectEditSurface` pointer state machine — ships under sub-entry `/direct-manipulation` (renderer-independent: no `three`, no DOM assumptions). The SVG affordance fragments ship on the main barrel. The drawn affordances are decoration by contract (`aria-hidden`, never a synthetic widget — see AGENTS.md); every operation has a native twin control on the consumer side | Kernel under `/direct-manipulation` (`grammar`, `session`, `hit-test`, `geometry`, `constants`, `useDirectEditSurface`); `EditHandle`, `EditGhostHandle`, `EditHeadingKnob`, `EditRemoveBadge` on the barrel |
